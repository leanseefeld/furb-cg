...
=======
	public Camera(Tela tela, boolean permiteControlar) {
		...
		// incAnguloY = 45;
>>>>>>> f21d8f0 Alteração para exibir a moto. 
...
>>>>>>> f21d8f0 Alteração para exibir a moto. 
	 ...
	 * 
	 * @param glu
	 *            GLU a ser configurado de acordo com as configurações desta
	 *            câmera.
	 * @return se a câmera pode precisar ser ajustada no próximo quadro (como
	 *         quando está sendo animada).
	 */
	public boolean atualizar(GLU glu) {
		final Poligono objObservado = moto == null ? tela.getArena() : moto;
		if (objObservado == null) {
		...
		final Ponto pontoObservado = objObservado.getBBoxTransformada()
				.getCentro();
		float anguloCameraYProximo = moto == null ? incAnguloY : moto
				.getAngulo() + 180;
		...
		// Calcula a rotação e aproximação em X
		double novoX = (distancia + DISTANCIA)
				* Math.cos(Math.toRadians(anguloCameraY));
		novoX += pontoObservado.x;

		// Calcula a rotação e aproximação em Z
		double novoZ = (distancia + DISTANCIA)
				* Math.sin(Math.toRadians(anguloCameraY));
		novoZ += pontoObservado.z;
		...
		double tangente = catetoOposto / catetoAdjascente;
		double anguloY = Math.atan(tangente);
		double novoY = (distancia / DISTANCIA * ALTURA + ALTURA)
				* Math.sin(anguloY);

		glu.gluLookAt(novoX, novoY, novoZ, pontoObservado.x, pontoObservado.y,
				pontoObservado.z, 0, 1, 0);
		double[] novaConfig = { novoX, novoY, novoZ, pontoObservado.x,
				pontoObservado.y, pontoObservado.z, 0, 1, 0 };
		boolean renderizarNovamente = !Arrays.equals(novaConfig, antigaConfig);
		antigaConfig = novaConfig;
		return renderizarNovamente;
	}
